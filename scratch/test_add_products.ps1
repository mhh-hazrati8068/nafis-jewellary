...
$token = Invoke-RestMethod -Uri "http://nafiseebadijewellery.com/api/auth/admin-login" -Method Post -Body $json -ContentType "application/json"
$headers = @{ "Authorization" = "Bearer $token" }

# Let's test force silver price update
try {
    $priceRes = Invoke-RestMethod -Uri "http://nafiseebadijewellery.com/api/admin/update-silver-price" -Method Post -Headers $headers
...
try {
    ...
    $curlOut = curl.exe -X POST "http://nafiseebadijewellery.com/api/admin/products" `
...
$stones = Invoke-RestMethod -Uri "http://nafiseebadijewellery.com/api/admin/stones" -Method Get -Headers $headers
...
if ($stoneId) {
    $curlRing = curl.exe -X POST "http://nafiseebadijewellery.com/api/admin/products?stoneId=$stoneId" `
        -H "Authorization: Bearer $token" `
...
$allProd = Invoke-RestMethod -Uri "http://nafiseebadijewellery.com/api/admin/products" -Method Get -Headers $headers
